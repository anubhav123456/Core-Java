...
# **📌 Sequential Processing vs Parallel Processing (Thread Class)**
...
* Has `executeWork()` that prints tasks with `Thread.sleep(100)`.

### **Worker2**

* Same pattern, slightly different implementation of handling exception.

### **Main**

* Calls `worker1.executeWork()` → waits for it to finish.
* Then calls `worker2.executeWork()`.
...
### **ParallelWorker1 & ParallelWorker2**

* Both extend `Thread`.
* Override `run()` to execute tasks in a loop.
...
# **3. Differences — Sequential vs Parallel Processing**
...
If you want, I can convert these notes into **flashcards**, **PDF**, **one-page revision sheet**, or **interview Q&A format**.
